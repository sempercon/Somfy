...
@interface SceneConfigurator_Homeowner (Private)
-(void)startTimer;
...
@implementation SceneConfigurator_Homeowner
...
-(void)viewWillAppear:(BOOL)animated
{
	...
	isAddDeleteScene = NO;
	[[AppDelegate_iPad sharedAppDelegate]SetHomeownerViewIndex:3];
...
-(void)LoadAllScenes:(NSMutableArray*)sceneArr
{
	...
	for(int i=0;i<[sceneArr count];i++)
	{
		...
		UILabel *sceneName = [[UILabel alloc] initWithFrame:CGRectMake(x+8, y, 120, 60)];
		...
		for(int j=0;j<[selectedScenesArray count];j++)
		{
			if(i==[[selectedScenesArray objectAtIndex:j] intValue])
			{
				...
				if([SupportedMaskFilterArray count]>i)
				{
					...
					{
						UIButton * customBtn = [[UIButton buttonWithType:UIButtonTypeCustom] retain];
						customBtn.frame = CGRectMake(subx, suby, 74, 74);
						...
						else
						{
							...
							{
								DeviceImage = [[DeviceIconMapper getSharedInstance]getSomfyDeviceImageBasedOnDeviceType:[[[deviceArr objectAtIndex:j]objectForKey:@"metaData"]intValue]];
							}
							else {
						...
						// Hint
						//UILabel *lbl3 = [[UILabel alloc]initWithFrame:CGRectMake(subx, suby+65, 120, 60)];
                        UILabel *lbl3 = [[UILabel alloc]initWithFrame:CGRectMake(subx-10, suby+80, 110, 50)];
						...
						for(int k=0;k<[[AppDelegate_iPad sharedAppDelegate].g_roomsArray count];k++)
						{
							if([[[deviceArr objectAtIndex:j] objectForKey:@"roomID"] isEqualToString:[[[AppDelegate_iPad sharedAppDelegate].g_roomsArray objectAtIndex:k]objectForKey:@"id"]])
							{
                                ...
                                {
                                    UILabel *roomNameLbl = [[UILabel alloc]initWithFrame:CGRectMake(subx-10, suby+80+50, 110, 50)];
						...
						customBtn1.frame = CGRectMake(subx, Newy, 40, 40);
		...
		for(int j=0;j<[selectedScenesArray count];j++)
		{
			...
			{
				UIButton * customBtn = [[UIButton buttonWithType:UIButtonTypeCustom] retain];
				customBtn.frame = CGRectMake(x+10, y, 118, 117);
				[customBtn setTag:i];
				...
				UILabel *activateLbl = [[UILabel alloc]initWithFrame:CGRectMake(x+20, y+60, 100, 50)];
				...
				activateLbl.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:14];
...
-(void)SceneEdit:(id)sender
{
	...
	//Store last scrollview position while
	//editing 
    ...
    if((xposition+500)>((TagVal*142)+((selCount-1)*320)))
    {
        int Currpos = xposition;
        rightOffset = CGPointMake(Currpos,0);
    }
    else if(((xposition+500)<((TagVal*142)+((selCount-1)*320))) && ((xposition+600)>((TagVal*142)+((selCount-1)*320))))
    {
        int Currpos = xposition+150;//TagVal*165;
        rightOffset = CGPointMake(Currpos,0);
       
    }
    else if(((xposition+600)<((TagVal*142)+((selCount-1)*320))) && ((xposition+700)>((TagVal*142)+((selCount-1)*320))))
    ...
    else
    {
		...
		int widthToAdd = (curWidth+320)-940;
	...
	if(sceneScrollView.contentSize.width < 940)
...
-(void)SceneClose:(id)sender
{
	...
	if(rightOffset.x < 100 || sceneScrollView.contentSize.width<940)
	{
		rightOffset.x = 0;
		rightOffset.y =0;
		[sceneScrollView setContentOffset:rightOffset animated:NO];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
	if(alertView.tag == 255)
	{
		...
		else
		{
            ...
            streamview =  skinView;
            // Add powerplay skinview as subview
...
{
	[[AppDelegate_iPad sharedAppDelegate].SceneConfigHomeviewController.view removeFromSuperview];
