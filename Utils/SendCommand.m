...
//ShowAlert when No internet connection available.
-(void)ShowAlert
{
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Network Problem" 
													message:@"There was an error in the Network.Please double check network reachability." 
												   delegate:nil 
...
{
	Network *reachManager = [Network sharedReachability];
...
-(BOOL) SendAPICommand :(NSString *) strCommand :(NSString *) postCommand :(id<ParserCallback>)delegate
{
	...
	if(![self IsConnectionAvailable])
	{
		if (Parserdelegate != nil && [Parserdelegate respondsToSelector:@selector(commandFailed::)]) {
			[Parserdelegate commandFailed:@"Network Problem" :@"There was an error in the Network.Please double check network reachability."];
		}
		return YES;
	}
...
-(void) parsetHttpResponse
{
	...
	else if(status == 554433)
	{
		if (Parserdelegate != nil && [Parserdelegate respondsToSelector:@selector(commandFailed::)]) 
		{
			[Parserdelegate commandFailed:@"SESSION EXPIRED" :@"You have been inactive for 10 minutes, please log in."];
		}
	}
	else if(status == 554444)
	{
		if (Parserdelegate != nil && [Parserdelegate respondsToSelector:@selector(commandFailed::)]) 
		{
			[Parserdelegate commandFailed:@"Error" :@"BOX_NOT_CONNECTED"];
		}
	}
	else if(status == FE_FAIL_DB_UNAFFECTED)
	{
		if (Parserdelegate != nil && [Parserdelegate respondsToSelector:@selector(commandFailed::)]) 
		{
			[Parserdelegate commandFailed:@"Error" :@"DB UNAFFECTED"];
		}
	}
...
-(void)parseData:(NSString *)strNode
{
	...
	{
		itemResultNodes = [rssParser nodesForXPath:[@"//" stringByAppendingString:@"instance"] error:nil];
		if([itemResultNodes count]==0)
		{
			itemResultNodes=[[rssParser rootElement]elementsForName:@"instance"];
		}
		for (CXMLElement *resultElement in itemResultNodes) 
		{
			NSMutableDictionary *Dict=[[NSMutableDictionary alloc]init];
			for(int counter = 0; counter < [resultElement childCount]; counter++) {
				NSArray *child = [resultElement attributes];
				for (CXMLNode *attribute in child)
...
-(int)parseStatus
{
	...
	for (CXMLElement *resultElement in itemResultNodes) 
	{
		...
		{
			if([[Dict objectForKey:@"status"] isEqualToString:@"SESSION_EXPIRED"])
				nStatus = 554433;
			else if([[Dict objectForKey:@"status"] isEqualToString:@"BOX_NOT_CONNECTED"])
				nStatus = 554444;
			else
				nStatus = [[Dict objectForKey:@"status"] intValue];
		}
...
static SendCommand *sharedInstance = nil;
