...
static Network *_sharedReachability;
...
+ (Network *)sharedReachability
{
    if (!_sharedReachability) {
        _sharedReachability = [[Network alloc] init];
        // Clients of Network will typically call [[Network sharedReachability] setHostName:]
...
{        
    ...
    // -scheduleOnRunLoop: will schedule only if network status notifications are enabled in the Network instance.
...
- (void)scheduleOnRunLoop:(NSRunLoop *)inRunLoop
{
    ...
    if ([[Network sharedReachability] networkStatusNotificationsEnabled] == NO) {
        return;
    }
